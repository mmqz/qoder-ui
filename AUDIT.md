...
## 后记：v3.3.2（2026-09-04，测试驱动补充修复）

本次常规测试验证（产物导入检查）发现审计未覆盖的盲区：**审计与完整性测试均只验证
"ESM/CJS 26 个导出键存在"，未验证取值** —— 实际 Node 环境下 21 个导出为 `undefined`
（模块把注册包进 window 守卫）。浏览器中完全正常，因此此前的浏览器审计无法发现。

- 修复：6 模块注册移至 globalThis、5 处全局访问加护栏（详见 README v3.3.2 节）
- 防回归：新增 tests/exports.test.mjs —— 键值双断言 + 干净子进程 SSR 导入验证
- 教训：**"SSR 安全" ≠ "不崩溃"，导出 API 面必须在所有目标环境可用或显式降级**
- 套件 52 → 57；版本 3.3.1 → 3.3.2
